...
> ⚠️ **등록은 운영자(JWT) 작업이다 — 셋업을 누가 할지 개발자에게 먼저 물어라.** "랭킹 붙여줘"
> 요청에 곧장 curl 안내만 하지 말고, ① 내가(에이전트) CLI 로 직접 컬럼+리더보드를 추가할지
> (→ **운영자 로그인 정보 필요**), ② 개발자가 대시보드에서 직접 할지 택1을 묻는다. ②면 무엇을
> 만들지 정확히 안내하고 게임 코드(조회)만 짠다. (SKILL.md "4.5 운영자 셋업" 참고.)
