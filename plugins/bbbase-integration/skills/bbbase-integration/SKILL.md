...
> **⚡ 게임 엔진이면 공식 SDK 를 먼저 확인하라.**
> - **Unity** — `Assets/BBBase/`(또는 `using BBBaseSdk;` / `BBBase.Init()`)가 보이면 SDK 설치됨.
>   `UnityWebRequest` 를 손수 짜지 말고 **SDK API + `references/unity-sdk.md`**.
> - **Godot** — `addons/bbbase/`(또는 autoload `BBBase` / `BBBase.init()`)가 보이면 SDK 설치됨.
>   `HTTPRequest` 를 손수 짜지 말고 **SDK API + `references/godot-sdk.md`**.
>
